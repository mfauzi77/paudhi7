...
const Sidebar = ({ activeView, setActiveView, isOpen, setIsOpen, horizontal = false, onNavigateToRegion }) => {
  ...
  if (horizontal) {
    ...
    const highPriorityAlerts = useMemo(() => (
      useIntegration ? [] : allActiveAlerts.filter(
        ...
        <div className="px-4 sm:px-6">
          <div className="flex items-stretch py-1 gap-2">
            <div className="flex flex-1 items-stretch w-full gap-2">
              {menuItems.map((item) => (
                <NavLink key={item.id} item={item} extraClasses="flex-1 justify-center text-center px-3 py-2 truncate" />
              ))}
            </div>
            <div className="relative flex items-center" ref={notificationsRef}>
              ...
              {isNotificationsOpen && (
                <NotificationDropdown
                  ...
                  onNavigate={(view) => { setActiveView(view); setIsNotificationsOpen(false); }}
                  onNavigateToRegion={(region) => { if (onNavigateToRegion) { onNavigateToRegion(region); } setIsNotificationsOpen(false); }}
                />
              )}
            </div>
          </div>
  ...
  return (
    ...
    <aside className={`fixed lg:relative inset-y-0 left-0 w-64 flex-shrink-0 bg-white dark:bg-slate-800 shadow-lg dark:shadow-slate-700/50 flex flex-col p-4 z-30 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
      <div className="flex items-center mb-6 px-2">
        <img src="/logo.png" alt="Logo Kemenko PMK" className="h-12 w-10 mr-3"/>
        <div className="text-gray-800 dark:text-slate-100">
            <p className="font-bold text-lg leading-tight">CERIA</p>
            <p className="text-xs text-gray-500 dark:text-slate-400">Sistem Pendukung Keputusan Cerdas</p>
        </div>
      </div>
