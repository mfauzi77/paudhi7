...
const PredictionChart = ({ data, domain, horizon }) => {
    ...
    return (
        <div>
            <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4">
                <h3 className="text-lg font-bold text-slate-800 mb-2 sm:mb-0">Proyeksi Risiko - {domain} ({horizon})</h3>
                 <div className="flex items-center space-x-4 text-xs text-slate-600">
                    <div className="flex items-center"><span className="w-3 h-0.5 bg-blue-500 mr-2" aria-hidden="true"></span>Data Aktual</div>
                    <div className="flex items-center"><span className="w-3 h-0.5 border-t-2 border-dashed border-indigo-500 mr-2" aria-hidden="true"></span>Data Prediksi</div>
                    <div className="flex items-center"><div className="w-3 h-3 bg-indigo-100 mr-2" aria-hidden="true"></div>Interval Kepercayaan</div>
                </div>
            </div>
            <div className="w-full overflow-x-auto">
                <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="min-w-[500px] h-auto" preserveAspectRatio="xMidYMid meet" aria-labelledby="chart-title">
                    <title id="chart-title">Grafik Proyeksi Risiko untuk {domain} selama {horizon}</title>
                    ...
                    {firstPredictedIndex >= 0 && (
                      <path 
                        ...
                        stroke="none"
                        aria-label="Interval kepercayaan prediksi"
                      />
                    )}
                    ...
                    {actualDataPoints.map((p, i) => (
                        <circle key={`actual-dot-${i}`} cx={getX(i)} cy={getY(p.actual)} r="3" className="fill-white stroke-blue-500" strokeWidth="2">
                          <title>Bulan: {p.month}, Data Aktual: {p.actual}</title>
                        </circle>
                    ))}
                    ...
                    {data.map((p, i) => (
                         ...
                         <circle key={`pred-dot-${i}`} cx={getX(i)} cy={getY(p.predicted)} r="3" className="fill-white stroke-indigo-500" strokeWidth="2">
                           <title>Bulan: {p.month}, Data Prediksi: {p.predicted} (Interval: {p.predicted_lower}-{p.predicted_upper})</title>
                         </circle>
                    ))}
                    ...
                    {firstPredictedIndex > 0 && (
                        <line 
                            ...
                            strokeWidth="1" 
                            strokeDasharray="2 2"
                            aria-label="Pemisah data aktual dan prediksi"
                        />
                    )}
                </svg>
            </div>
