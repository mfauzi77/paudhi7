...
const TopMovers = ({ data }) => {
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-sm">
            <h3 className="text-lg font-bold text-slate-800 mb-4">Perubahan Terbesar</h3>
            <div>
                <h4 className="text-xs font-semibold text-red-600 uppercase tracking-wider mb-2">Eskalasi Risiko Tertinggi</h4>
                <ul className="divide-y divide-slate-100">
                    {topIncreases.map(item => <MoverItem key={`inc-${item.id}`} item={item} />)}
                </ul>
            </div>
            <div className="mt-4">
                <h4 className="text-xs font-semibold text-emerald-600 uppercase tracking-wider mb-2">Perbaikan Terbaik</h4>
                 <ul className="divide-y divide-slate-100">
