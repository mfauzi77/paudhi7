...
const RiskMomentumQuadrant = ({ regions, onRegionSelect, selectedRegionId }) => {
    const width = 600;
    const height = 450;
    const padding = 60;
    ...
    const getX = (score) => padding + ((score - xMin) / (xMax - xMin)) * (width - padding * 2);
    const getY = (trend) => (height - padding) - ((trend - yMin) / (yMax - yMin)) * (height - padding * 2);

    const xCenterLine = getX(xCenter);
    const yCenterLine = getY(yCenter);

    const getQuadrant = (score, trend) => {
        ...
        <div>
            ...
            <div className="w-full overflow-x-auto">
                <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto" role="figure" aria-labelledby="chart-title-quadrant">
                    ...
                    <text x={width - padding - 10} y={height - padding - 20} textAnchor="end" className={`text-sm font-bold fill-current ${quadrantInfo[4].style.text}`}>{quadrantInfo[4].description}</text>
