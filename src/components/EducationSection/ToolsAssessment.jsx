...
const ToolsCard = React.memo(({ item, onItemClick }) => {
  ...
  return (
    <article 
      className="group bg-white rounded-2xl border border-gray-200 overflow-hidden hover:shadow-xl hover:-translate-y-2 transition-all duration-300 cursor-pointer focus-within:ring-2 focus-within:ring-emerald-500"
      onClick={handleClick}
    ...
    >
      ...
      <div className="p-6">
        ...
        <button 
          className="w-full bg-emerald-600 text-white py-3 rounded-lg font-semibold hover:bg-emerald-700 transition-colors duration-300 flex items-center justify-center gap-2 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2"
          aria-label={`Download tool ${item.title}`}
        >
          <i className="fas fa-download" aria-hidden="true"></i>
          Download Tool
